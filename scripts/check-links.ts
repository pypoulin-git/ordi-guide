/**
 * check-links.ts — Verify all product URLs in catalogue.json (TypeScript version)
 *
...
import type { CatalogueData, CatalogueProduct } from '../src/types/catalogue';
...
async function checkUrl(url: string): Promise<CheckResult> {
  ...
  try {
    ...
    const message = e.name === 'AbortError' ? 'Timeout (5s)' : e.message;
...
async function main(): Promise<void> {
  ...
  const catalogue: CatalogueData = JSON.parse(raw);
  ...
  const results = await runWithConcurrency(
    ...
    async (product, i) => {
      ...
      const statusStr = result.status?.toString() ?? 'ERR';
