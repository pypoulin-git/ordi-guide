      ...
      <section className="section" style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #f8fafc 60%, #ecfeff 100%)' }}>
        <div className="container text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-semibold mb-6"
            ...
            Guide indépendant · Sans publicité · Vraiment simple
          </div>
      ...
      <section className="section" style={{ paddingTop: '2rem', paddingBottom: '2rem', background: '#f8fafc' }}>
        <div className="container max-w-2xl mx-auto text-center">
          ...
          <p className="text-xs mt-3" style={{ color: '#94a3b8' }}>
            Tu peux changer à tout moment via le bouton dans le menu
          </p>
        </div>
      </section>
            ...
            {(['cpu', 'ram', 'ssd', 'gpu', 'battery', 'screen'] as const).map(comp => {
              ...
              return (
                <div key={comp} className="card text-center" style={{ padding: '1rem' }}>
                  ...
                  <div className="font-semibold text-sm" style={{ color: '#0f172a' }}>{analogy.name}</div>
                  <div className="text-sm" style={{ color: '#64748b' }}>{analogy.short}</div>
                  <div className="text-xs uppercase tracking-wider mt-1" style={{ color: '#94a3b8' }}>
      ...
      <section className="section" style={{ background: 'white' }}>
        <div className="container max-w-2xl mx-auto">
          ...
          <p className="text-center mb-6" style={{ color: '#64748b' }}>
            Dis-nous ce que tu cherches en une phrase, notre assistant te donne une recommandation instantanée
          </p>
          <SearchBar />
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {features.map(f => (
              <Link key={f.href} href={f.href} className="card block hover:no-underline group"
                ...
                <p className="leading-relaxed mb-5" style={{ color: '#475569', fontSize: '1rem' }}>{f.desc}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {reassurances.map(r => (
              <div key={r.label} className="text-center">
                <div className="font-semibold mb-1" style={{ color: '#0f172a', fontSize: '1rem' }}>{r.label}</div>
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <details key={i} className="card cursor-pointer" style={{ padding: '1.25rem 1.5rem' }}>
                ...
                <p className="mt-3 leading-relaxed" style={{ color: '#475569', fontSize: '1rem' }}>{faq.a}</p>
              </details>
            ))}
          </div>
        </div>
